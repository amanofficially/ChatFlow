...
import {
  Check, CheckCheck, Copy, Trash2, MoreHorizontal, SmilePlus,
  ...
  return (
    ...
    >
      ...
      <img
        ...
        style={{
          maxWidth: "min(92vw, 880px)", maxHeight: "82dvh",
          opacity: loaded ? 1 : 0, transform: `scale(${scale})`,
          cursor: scale > 1 ? "zoom-out" : "zoom-in", WebkitTouchCallout: "default",
        }}
        onClick={(e) => e.stopPropagation()}
      />
      {loaded && scale === 1 && (
...
function ContextMenu({ isOwn, hasText, onCopy, onDelete, onClose, anchorRef, position }) {
  ...
  return (
    ...
    >
      {hasText && onCopy && (
        <button onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); onCopy(); }} style={{ touchAction: "manipulation" }} className="context-menu-item">
      ...
      {isOwn && (
        <button onMouseDown={(e) => { e.preventDefault(); e.stopPropagation(); onDelete(); }} style={{ touchAction: "manipulation" }} className="context-menu-item danger">
...
function ReactionBar({ isOwn, currentReaction, onReact, onClose, triggerRef }) {
  ...
  return (
    <div
      ...
      style={{ background: "var(--bg-secondary)", animation: "reactionBarIn 0.2s cubic-bezier(0.34,1.56,0.64,1)", boxShadow: "0 8px 32px rgba(0,0,0,0.18)" }}
      onClick={(e) => e.stopPropagation()}
    >
      {REACTIONS.map((emoji) => (
        <button
          ...
          className={`text-xl w-9 h-9 flex items-center justify-center rounded-xl transition-all duration-150 active:scale-90
            ${currentReaction === emoji ? "bg-[var(--brand)]/15 scale-110 ring-2 ring-[var(--brand)]/30" : "hover:bg-[var(--bg-tertiary)] hover:scale-125"}`}
        >
          {emoji}
        </button>
      ))}
...
function ReactionSummary({ reactions, isOwn, onPillClick }) {
  ...
  return (
    <div className={`absolute -bottom-4 flex items-center gap-0.5 z-10 ${isOwn ? "right-2" : "left-2"}`} style={{ animation: "reactionPop 0.22s ease-out" }}>
      {Object.entries(counts).map(([emoji, count]) => (
        <button key={emoji} onMouseDown={(e) => e.preventDefault()} onClick={(e) => { e.stopPropagation(); onPillClick(); }} style={{ touchAction: "manipulation" }}
...
function MessageContent({ message, isOwn, onImageClick }) {
  ...
  useEffect(() => {
    ...
    const handler = (e) => { if (!fileMenuRef.current?.contains(e.target)) setShowFileActions(false); };
    const t = setTimeout(() => {
      document.addEventListener("mousedown", handler);
      document.addEventListener("touchstart", handler, { passive: true });
    }, 50);
  ...
  if (message.type === "image") {
    return (
      <div className="relative group/img">
        {!imgLoaded && !imgError && (
          <div className="rounded-xl animate-pulse flex items-center justify-center" style={{ width: 200, height: 140, background: "rgba(128,128,128,0.15)" }}>
        ...
        {!imgError ? (
          ...
          >
            ...
            />
            <div className="absolute inset-0 rounded-xl flex items-center justify-center opacity-0 group-hover/img:opacity-100 transition-opacity" style={{ background: "rgba(0,0,0,0.25)" }}>
          ...
          <div className="rounded-xl flex items-center justify-center text-xs opacity-60 px-4 py-3" style={{ background: "rgba(128,128,128,0.1)", minWidth: 120 }}>
            Image unavailable
          </div>
        )}
      </div>
      ...
      <div className="relative" ref={fileMenuRef}>
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className={`text-[11px] mt-0.5 ${isOwn ? "text-white/55" : "text-[var(--text-muted)]"}`}>Tap to open · download</p>
          </div>
          <Download size={14} className="flex-shrink-0 opacity-50" />
        </button>
        {showFileActions && (
          <div className="absolute bottom-full mb-2 left-0 z-50 py-1.5 min-w-[180px] shadow-2xl rounded-2xl overflow-hidden"
            style={{ background: "var(--bg-secondary)", border: "1px solid var(--border)", animation: "contextMenuIn 0.16s ease-out", boxShadow: "0 12px 40px rgba(0,0,0,0.2)" }}>
            {isPdf && (
              <a href={url} target="_blank" rel="noopener noreferrer" onClick={() => setShowFileActions(false)}
                ...
                <ExternalLink size={14} className="text-[var(--text-muted)]" /><span>Open PDF</span>
              </a>
            )}
            <a href={url} download={fileName} onClick={() => setShowFileActions(false)}
              className="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm text-[var(--text-primary)] hover:bg-[var(--bg-tertiary)] transition-colors">
              <Download size={14} className="text-[var(--text-muted)]" /><span>Download</span>
            </a>
          </div>
        )}
      </div>
...
export default function MessageBubble({ message, isOwn, showAvatar, sender }) {
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      try {
        const el = Object.assign(document.createElement("textarea"), { value: message.content, style: "position:fixed;opacity:0" });
  ...
  const handleTouchMove = useCallback((e) => {
    ...
    if (dx > 8 || dy > 8) { clearTimeout(longPressTimer.current); isPressingRef.current = false; }
  }, []);
        ...
        <div className="flex items-end gap-2 w-full flex-row-reverse">
          <div className="flex flex-col gap-0.5 items-end min-w-0">
            <div className="message-bubble-out break-words whitespace-pre-wrap opacity-70 relative pb-4">
              ...
              <span className="absolute bottom-1 right-2 text-[9px] text-white/50 leading-none">{formatMessageTime(message.createdAt)}</span>
            </div>
            <div className="px-1">
  ...
  return (
    <>
      ...
      <div className={`flex flex-col ${isOwn ? "items-end" : "items-start"} mb-0.5`} style={{ marginBottom: hasReactions ? "0.875rem" : undefined }}>
        <div className={`flex items-end gap-2 group w-full ${isOwn ? "flex-row-reverse" : ""}`}>
          ...
          <div className={`flex flex-col gap-0.5 min-w-0 ${isOwn ? "items-end" : "items-start"}`}>
            <div className="relative flex items-center gap-1 max-w-full">
              {!touch && (
                <div className={`flex items-center gap-0.5 transition-opacity duration-150 flex-shrink-0
              ...
              <div className="relative" ref={bubbleRef}>
                <div
                  className={`relative ${isOwn ? "message-bubble-out" : "message-bubble-in"} break-words whitespace-pre-wrap animate-slide-up`}
                  style={{ paddingBottom: "1.25rem", minWidth: displayMessage.type === "file" ? "180px" : "60px" }}
                ...
                >
                  <MessageContent message={displayMessage} isOwn={isOwn} onImageClick={setLightboxSrc} />
                  ...
                  {showReactions && (
                    <ReactionBar isOwn={isOwn} currentReaction={myReaction} onReact={handleReact} onClose={closeAll} triggerRef={smileBtnRef} />
                  )}

                  {showMenu && (
                    <ContextMenu isOwn={isOwn} hasText={message.type === "text"} onCopy={message.type === "text" ? handleCopy : null} onDelete={handleDelete} onClose={closeAll} anchorRef={menuBtnRef} position={menuPosition} />
                  )}
                </div>

                <ReactionSummary reactions={reactions} isOwn={isOwn} onPillClick={openReactions} />
              </div>
            </div>
